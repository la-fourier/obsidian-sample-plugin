...
// TODO debug (maybe set logs) renaming, docs schreiben, github repo, settings, clean code, integrate db dataview request  | proof extraction
...
export default class MyPlugin extends Plugin {
	...
	link_literature(clipboard: string) {
		...
		const outline_link = "lit-outline/" + file_link.split(".")[0] + "-outline.md";

		// while (!this.app.vault.getFileByPath(imgpath)) {
		// 	await setTimeout(()=>_, 500);
		// }                                                   sollte nicht gebraucht werden da autocopy ja automatisch passiert

		console.log("auf imgpath gewartet", imgpath);

		const newpath = "assets/" + file_link.split(".")[0] + "/" + imgpath.split("/")[1].replace(".", properties + ".");
		...
		console.log(this.app.vault.getFileByPath(newpath));

		var { text: text } = extractTextFromImage(this.app.vault.getFileByPath(newpath)!.path);
		const { env: env, tag: tag } = env_tags_by_text(text);
		text = ">[!" + env + "]" + text
		text = text.split("\n").join("\n> ")

		console.log("nach xtract")
